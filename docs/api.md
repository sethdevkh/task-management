...
REST contract for the Task Management MVP. Web and API are separate origins. Standup and dashboard endpoints are not in this phase.
...
RBAC for these paths is covered by `TaskApiTest` in the `restapi` CI job. There is still no standup or dashboard API.
